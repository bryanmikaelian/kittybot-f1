...
this.processCommand = function processCommand(room, command, callback){        
  ...
  if (command === "/sifters" || command === "/crs") {
    ...
    this.getAll(options, function(sifters){
      if (sifters.length === 0) {
          callback("Meow. There are no open sifters.");
      }
      else {
        for (var i = 0; i < sifters.length; i++) {
          issues.push(sifters[i]['number']);
        };
        callback("The following " + type + " are open: " + issues.join(", ") + ". Type " + cmd + " <number> to see more info.");
      }
    });
  }
      ...
      if (sifters.length === 0) {
        ...
        for (var i = 0; i < sifters.length; i++) {
          ...
          if (sifters[i]['number'].toString() === sifterNumber) {
            sifter = sifters[i];
            callback("Sifter #" + sifter['number'] + ": " + sifter['subject']);
            callback("Assigned to: " + sifter['assignee_name']);
            callback("State: " + sifter['category_name']);
            break;
          }
        };
